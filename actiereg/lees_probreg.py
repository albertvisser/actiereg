...
def modify_data(pnaam):
    ...
    for item in data:
        ...
        ## con.commit()
        cmd = "select id,start,text from {0}_event where actie_id = ?".format(pnaam)
        ...
        for start,text in actie.events:
            ...
            try:
                ...
                print "Geen sql events (meer) voor actie {0}".format(sql_actie_id)
                break
            cmd = "update {0}_event set start = ? where id = ?".format(pnaam)
